...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val ivtext = findViewById<TextView>(R.id.tvTop)
        val ivimage = findViewById<ImageView>(R.id.ivTop)
        var maxEntry=0
        for (i in 1 until voteResult!!.size) {
            if (voteResult!![maxEntry] < voteResult!![i]) maxEntry = i
        }
        ivtext.setText(imageName[maxEntry])
        ivimage.setImageResource(imageFileId[maxEntry])
